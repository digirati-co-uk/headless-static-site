...
test("publishes fresh dev output, preserves the last successful response on failure, and updates aggregates after edits", async () => {
  ...
  const config: any = {
    ...
    run: ["extract-part-of-collection"],
  ...
  try {
    ...
    await writeFile(
      join(root, "content/b.json"),
      JSON.stringify({ ...manifest("Second"), id: "https://example.org/b" })
    );
    ...
    await expect(stat(join(root, ".iiif/dev/build/manifests/b/manifest.json"))).rejects.toMatchObject({
      code: "ENOENT",
    });
    ...
    const edited = await server.request("/manifests/a/manifest.json", {
      ...
      body: JSON.stringify(manifest("Edited")),
    });
    ...
    const created = await server.request("/create?json=true", {
      method: "POST",
      body: new URLSearchParams({ slug: "nested/new", store: "objects" }),
    });
    ...
    const invalid = await server.request("/create?json=true", {
      method: "POST",
      body: new URLSearchParams({ slug: "../escape", store: "objects" }),
    });
...
test("reloads standalone config and updates watch roots when a store is added", async () => {
  ...
  try {
    ...
    const manifest = (label: string) =>
      JSON.stringify({ id: "https://example.org/other", type: "Manifest", label: { en: [label] }, items: [] });
    const config: any = {
      server: { url: "http://localhost:7111" },
      run: [],
...
test("failed buffered writes stay pending and can be retried", async () => {
  ...
  try {
    ...
    await files.saveJson("pending.json", { edited: true });
    vi.spyOn(files, "writeFile").mockRejectedValueOnce(new Error("Disk temporarily unavailable"));
    expect((await files.saveAll()).failedToWrite).toHaveLength(1);
    expect(files.openJsonChanged.get(join(root, "pending.json"))).toBe(true);
    expect((await files.saveAll()).failedToWrite).toEqual([]);
    expect(JSON.parse(await readFile(join(root, "pending.json"), "utf8"))).toEqual({ edited: true });
  } finally {
    await rm(root, { recursive: true, force: true });
  }
});
