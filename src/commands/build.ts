...
const CACHE_IO_ERROR_CODES = new Set([
  "EACCES",
  "EPERM",
  "EROFS",
  "ENOSPC",
...
async function buildInternal(
  ...
  buildConfig.files.writtenFiles.clear();
  buildConfig.files.writtenHashes.clear();
  ...
  if (buildConfig.options.emit) {
    ...
    const { failedToWrite, timings: saveTimings } = await fileHandler.saveAll(false, buildConfig.concurrency.write);
    Object.assign(timings, saveTimings);
      ...
      await Promise.all(
        entries.map(async (entry) => {
          ...
          await inventoryQueue.add(async () => {
            // Generated bytes were already hashed by the writer. Copies and external
            // producers still need a read from the completed output.
            let digest = buildConfig.files.writtenHashes.get(filePath);
            if (!digest) {
              const data = await buildConfig.files.fs.promises.readFile(filePath);
              digest = { bytes: data.byteLength, sha256: createHash("sha256").update(data).digest("hex") };
            }
            inventory.push({ path: outputPath, ...digest });
          });
        })
      );
    };
    await visit(resolvedOutputRoot);
    timings["inventory"] = performance.now() - inventoryStarted;
    const descriptorsStarted = performance.now();
    if (buildConfig.config.output?.includeResourceDescriptors) {
      const message =
        "Resource descriptors are enabled; generating per-resource artifact metadata adds build time and output size.";
      if (!onEvent) console.warn(message);
      await onEvent?.({ type: "diagnostic", level: "warning", message, at: new Date().toISOString() });
      ...
      // Remove stale opt-in output in dev and partial builds too.
      await buildConfig.files.remove(join(buildConfig.buildDir, "meta", "resource-descriptors.json"));
    }
    timings["descriptors"] = performance.now() - descriptorsStarted;
    ...
    timings["save-files"] = Date.now() - saveStartedAt;
    await onEvent?.({
      type: "phase-completed",
      phase: "save-files",
  ...
  if (manifest && Object.keys(extractions.cacheStats || {}).length) {
    // Entries saved by a failed output build must not become reusable cache hits.
    await buildConfig.files.writeFile(
      join(buildConfig.cacheDir, EXTRACTION_CACHE_COMMIT),
      buildConfig.extractionCacheGeneration
    );
  }
